# Spec: Autograd Engine

## Purpose

Defines the automatic differentiation (autograd) engine for trainadillo. The engine
tracks operations on tensors with `requires_grad=True`, constructs a dynamic computation
graph via `GradFn` nodes, and propagates gradients backward through that graph when
