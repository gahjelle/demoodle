...
Defines how text corpora are loaded from bundled package data and exposed as `Corpus` artifacts for use in training and inference pipelines. Corpora are stored as plain text files inside the `demoodle.data` package and accessed via `importlib.resources`.
