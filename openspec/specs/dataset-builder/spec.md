...
Encodes a `Corpus` artifact into a `Dataset` artifact by mapping each character
to a token id via a `TokenizerProtocol` implementor. Integrates as a standard
pipeline stage so that downstream training stages receive a flat 1D token tensor.
