...
The system SHALL provide `BigramArchitecture` in `demoodle.architectures.bigram` with `vocab_size: int` bound at construction. It SHALL implement `init_state(rng: RNG) -> Policy` and `forward(policy: Policy, tokens: Seq) -> Output`, satisfying `ArchitectureProtocol` structurally.
