...
The `RNG` capability provides a purely functional, reproducible random number generation abstraction. It is designed as an immutable value type that can be deterministically split and used to seed PyTorch generators, enabling reproducible experiments without relying on global random state.
