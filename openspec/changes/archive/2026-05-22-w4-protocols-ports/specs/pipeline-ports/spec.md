...
The system SHALL provide a `Stage` frozen dataclass with fields `name: str`, `needs: list[str]`, `produces: list[str]`, and `run: Callable[[dict[str, Artifact], RNG], dict[str, Artifact]]`. `Stage` is not a Protocol; it is a concrete, immutable value.
