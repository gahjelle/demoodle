...
from demoodle.architectures.bigram import BigramArchitecture, BigramModel, _sample
...
def test_sample_top_k_1_returns_argmax() -> None:
    logits = torch.tensor([0.1, 5.0, 0.2, 0.3])
    result = _sample(logits, temperature=1.0, top_k=1, top_p=None)
    assert int(result.item()) == int(logits.argmax().item())


def test_sample_top_p_zero_returns_argmax() -> None:
    logits = torch.tensor([0.1, 5.0, 0.2, 0.3])
    result = _sample(logits, temperature=1.0, top_k=None, top_p=0.0)
    assert int(result.item()) == int(logits.argmax().item())


def test_sample_no_filters_samples_full_range() -> None:
    torch.manual_seed(42)
    logits = torch.ones(10)
    draws = {
        int(_sample(logits, temperature=1.0, top_k=None, top_p=None).item())
        for _ in range(200)
    }
    assert len(draws) > 5
